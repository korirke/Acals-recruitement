...
import type { UpdateCompanyDto } from "@/types/recruitment/company.types";
...
export default function EditCompanyPage() {
  ...
  const { myCompany, loading, fetchMyCompany, updateMyCompany } = useCompanies();
  ...
  useEffect(() => {
    if (myCompany) {
      setFormData({
        ...
        website: myCompany.website || null,
        description: myCompany.description || null,
        logo: myCompany.logo || null,
        coverImage: myCompany.coverImage || null,
        socialLinks: {
          linkedin: myCompany.socialLinks?.linkedin || null,
          twitter: myCompany.socialLinks?.twitter || null,
          facebook: myCompany.socialLinks?.facebook || null,
        },
      });
    }
  }, [myCompany]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!myCompany || !validateForm()) return;
    ...
    const success = await updateMyCompany(submitData);
    setSaving(false);

    if (success) {
      showToast({ type: 'success', title: 'Company Updated', message: 'Your company profile has been updated successfully' });
    ...
    return (
      <Card>
        <CardContent className="py-12 sm:py-16 text-center">
          ...
          <h3 className="text-base sm:text-lg font-semibold mb-2 text-neutral-900 dark:text-white">No Company Found</h3>
          <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 mb-4">Please complete the company setup wizard</p>
          <Button asChild size="sm">
            <Link href="/recruitment-portal/setup/company">Complete Setup</Link>
          </Button>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-neutral-900 dark:text-white">Edit Company Profile</h1>
          <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400">Update your company information</p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row justify-between gap-3">
              <Button type="button" variant="outline" size="sm" onClick={() => router.push("/recruitment-portal/dashboard")} disabled={saving || uploadingLogo || uploadingCover} className="w-full sm:w-auto">
                Cancel
              </Button>
